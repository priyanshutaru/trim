...
import { DOMAIN_NAME, VALID_URL, PASSWORD } from "../config/constants";
...
export const stripUrl = async (req, res, next) => {
  ...
  if (new Date(long_url).getTime() < new Date().getTime())
    return respondWithWarning(res, 400, "Expiry date must be in the future");
  const schema = Joi.object({
    ...
    custom_url: Joi.string()
      ...
      .error(
        new Error(
          "custom URL must contain only alphanumeric, period(.), hyphen(-), underscore(_) and tilde(~) characters"
        )
      ),
  });
  ...
  const { error } = await schema.validate(
    { url: long_url, expiry: new Date(expiry_date), custom_url },
    validationOptions
  );
  if (error) {
...
export const validateOwnDomain = (req, res, next) => {
  ...
  if (
    ...
    const result = respondWithWarning(
      res,
      400,
  ...
  if (customUrl) {
    ...
    if (retrievedClip) {
      ...
      if (retrievedClip.expiry_date && retrievedClip.expiry_date < Date.now()) {
        ...
        return next();
      } //If the custom url is in use and not expired, respond with error.
      else
        return respondWithWarning(
          res,
          409,
          "Custom URL already in use. Please try another"
        );
    }
  }
...
export const passwordMatches = async (req, res, next) => {
  ...
  if (password) {
    ...
    return respondWithWarning(res, 401, "Please enter password.");
  }
